...
/**
 * <pre>
 *     Refer to io.netty.handler.codec.http.HttpObjectAggregator
 *     适配Puppy的HttpRequest和HttpResponse
 * </pre>
 *
 * @author wangcheng
 * @since V0.7.0 on 17/1/22.
 */
public class HttpObjectAggregator extends MessageToMessageDecoder<HttpObject> {
    ...
    protected void decode(final ChannelHandlerContext ctx, HttpObject msg, List<Object> out) throws Exception {
        ...
        if (msg instanceof HttpMessage) {
            ...
            if (msg instanceof HttpRequest) {
                ...
                this.currentMessage = currentMessage = new AggregatedFullHttpRequest(header, ctx.alloc().compositeBuffer(maxCumulationBufferComponents), null);
            } else if (msg instanceof HttpResponse) {
                HttpResponse header = (HttpResponse) msg;
                this.currentMessage = currentMessage = new AggregatedFullHttpResponse(header, Unpooled.compositeBuffer(maxCumulationBufferComponents), null);
            } else {
                throw new Error();
            }
    ...
    public abstract static class AggregatedFullHttpMessage implements ByteBufHolder, FullHttpMessage {
    ...
    private static final class AggregatedFullHttpRequest extends AggregatedFullHttpMessage implements FullHttpRequest {
        ...
        public FullHttpRequest setMethod(HttpMethod method) {
            ((io.netty.handler.codec.http.HttpRequest) message).setMethod(method);
            return this;
        }

        @Override
        public FullHttpRequest setUri(String uri) {
            ((io.netty.handler.codec.http.HttpRequest) message).setUri(uri);
            return this;
        }

        @Override
        public HttpMethod getMethod() {
            return ((io.netty.handler.codec.http.HttpRequest) message).getMethod();
        }

        @Override
        public String getUri() {
            return ((io.netty.handler.codec.http.HttpRequest) message).getUri();
        }
