...
import cn.wantedonline.puppy.httpserver.component.SystemChecker;
import cn.wantedonline.puppy.httpserver.util.command.CommandService;
...
public class SystemMonitor {
    ...
    public static boolean initThreadCpuTimeMonitor(int interval) {
        ...
        try {
            cpuTimeEnabled = threadMXBean.isThreadCpuTimeEnabled();
            ...
            ConcurrentUtil.getWatchdog().scheduleWithFixedDelay(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        calcThreadsCpuTimeTmp = calcThreadsCpuTimeTmp;
                        ...
                        if (printInfo && logger.isInfoEnabled()) {
                            ...
                            if (sort.size() > 0 && sort.get(0).getValue() > 0) {
                                ...
                                for (int i = 0; i < 10; i++) {
                                    ...
                                    tmp.append(String.format(fmt, id, info.getThreadState(), info.getBlockedCount(), info.getWaitedCount(), HumanReadableUtil.timeSpan(cpuTime / 1000), lastCpuTime,
                                            info.getThreadName()));
                                }
                                logger.info("\n{}",tmp);
                            }
        ...
        logger.info("ThreadCpuTimeMonitor     OFF,cpuTimeSupported{},cpuTimeEnabled:{}", cpuTimeSupported, cpuTimeEnabled);
        return false;
    }
    ...
    public static boolean initLoadAverageMonitor(int interval) {
        ...
        if (mx.getSystemLoadAverage() < 0) {
            logger.info("LoadAverageMonitor      OFF,because jvm can't stat loadaverage in os:{}/arch:{}", mx.getName(), mx.getArch());
            return false;
        }
        logger.info("LoadAverageMonitor      ON,interval:{}sec,warnThreshold:{},errorThreshold:{}", new Object[] {
                interval,
            ...
            public void run() {
                try {
                    ...
                    if (load < SystemChecker.getInstance().getLoadMonitorWarnThreshold()) {
                        logger_loadaverage.info("load average:{}", load);
                        ...
                        logger_loadaverage.error("load average:{}\n{}", load, loadCurrentStatus.toString());
                        loadAverageHigh = true;
                    } else {
                        MDC.put("mailTitle", "LOAD AVERAGE:" + load);
                        logger_loadaverage.warn("load average:{}", load);
                        loadAverageHigh = false;
                    }
                } catch (Throwable e) {
                    logger.info("", e);
    ...
    public static boolean initGarbageCollectMonitor(int interval) {
        ...
        if (gcs.size() == 2) {// TODO:暂时处理gc为两个的情况
            ...
            if (youngGc.getMemoryPoolNames().length < fullGc.getMemoryPoolNames().length) {// TODO:暂时认为fullGc的memoryPoolName
                logger.info("GarbageCollectMonitor   ON,interval:{}sec,yongGc:{},fullGc:{}", new Object[] {
                        interval,
                        youngGc.getName(),
                        fullGc.getName()
                });
            ...
            logger.info("GarbageCollectMonitor   OFF,because yongGc.memoryPool:[{}] < fullGc.memroyPool:{}", Arrays.toString(youngGc.getMemoryPoolNames()), Arrays.toString(fullGc.getMemoryPoolNames()));
        } else {
            StringBuilder sb = new StringBuilder();
            int i = 1;
            ...
            logger.info("GarbageCollectMonitor   OFF,because garbageCollectorMXBeans.size<>2:{}", sb);
        }
        return false;
    }
    ...
    public static boolean initThreadPoolMonitor(int interval, final int queueThreshold, final ExecutorService... executorServices) {
        ...
        logger.info("ThreadPoolMonitor       ON,interval:{}sec,queueThreshold:{},checkExecutors:{}", new Object[] {
                interval,
                queueThreshold,
                ...
                if (e instanceof ThreadPoolExecutor) {
                    ...
                    if (executor.getQueue().size() > queueThreshold) {
                        ...
                        if (ConcurrentUtil.getNamedExecutorThreadFactory(e) == null) {
                            logger.error("ThreadPool WARNNING:{}\n", ConcurrentUtil.getExecutorInfo(e));
                        } else {
